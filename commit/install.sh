...
apt-get -y install openssh-client openssh-server make openjdk-7-jre ant git r-base php5 tomcat7 unzip curl libcairo2-dev libxt-dev postgresql-9.3 postgresql-contrib-9.3
...
#!/usr/bin/Rscript
install.packages(c('Rserve','Cairo','MASS','stringr','ggplot2','plyr','reshape2','gplots','data.table','matrixStats', 'Hmisc', 'foreach', 'doParallel', 'reshape', 'fastcluster', 'dynamicTreeCut', 'survival'), repos='http://cran.us.r-project.org')
...
# necessary as "make -C env ubuntu_deps_regular" is faulty cloning tranSMART-ETL -> always timeout
...
# installing grails 2.3.11 again to start transmart
gvm rm grails 2.3.7
gvm install grails 2.3.11
[[ -s "/root/.gvm/bin/gvm-init.sh" ]] && source "/root/.gvm/bin/gvm-init.sh"
grails run-app
